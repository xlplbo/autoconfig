...
RC_EXT_OR_ELF = ('.dll', '.exe', ".so", ".a", 'kg_goddess',
                 'kg_bishop', 'so2relay', 'so2gamesvr')
...
RC_ROUTER_LIST = (
    'engine.dll', 'lualibdll.dll', 'router.exe', 'verify_up2date.exe')
RC_MAX_SIZE = 8192  # MB
...
class AutoConfig(object):
    ...
    def __init__(self, config, log):
        ...
        self.configParser.readfp(
            codecs.open(config, "r", self._getFileCoding(config)))
            ...
            for x in ['eth0', 'wlan0', 'lo']:
                try:
                    localip = socket.inet_ntoa(
                        fcntl.ioctl(s.fileno(), 0x8915, struct.pack('256s', x[:15]))[20:24])
                    self.log.info(
                        "assign requested address %s --> %s" % (x, localip))
                    return localip
                except Exception, e:
        ...
        if self._checkYesOrNO("if the IP is false, please correct it! do you want to modify?(y/N)"):
            while True:
                ...
                if len(ipformat) != 4 or len(filter(lambda x: x >= 0 and x <= 255, map(int, filter(lambda x: x.isdigit(), ipformat)))) != 4:
                    self.log.error(
                        "does not conform to the rules, input again!")
                    continue
    ...
    def _handlerError(self, func, path, excinfo):
        ...
        func(path)
        self.log.info("Exception handling: %s %s" %
                      (getattr(func, '__name__'), path))
    ...
    def packLibrary(self, name):
        ...
        self.log.warn(
            "please compile your project at first, and then run packaged program!")
        ...
        for pathlv in pathlist:
            ...
            if not os.path.isdir(pathlv):
                self.log.error(
                    "path '%s' is wrong, please check '%s'" % (pathlv, self.config))
                continue
    ...
    def _getFileCount(self, path, blacklist):
        ...
        for root, dirs, files in os.walk(path):
            if len([x for x in blacklist if x in root]) > 0:
                continue
            count += files.__len__() - \
                len([x for x in files for y in blacklist if y in x])
        return count
        ...
        for root, dirs, files in os.walk(src):
            ...
            for file in [x for x in files if x not in blacklist]:
                ...
                if step > 0:
                    ...
                    self.log.info("progress %d%%" % 100)
        self.log.info("actually %d file(s) have been copied" %
                      (filecount - lnkfilecount))

    def copyFolder(self, src, dst, blacklist=[]):
        if not os.path.exists(src):
            self.log.error(
                "can not find '%s', please check '%s' file" % (src, self.config))
        if not os.path.exists(dst):
            self.log.error(
                "can not find '%s', please check '%s' file" % (dst, self.config))
    ...
    def _extractFiles(self, path, list):
        ...
        for v in list:
            if v not in extFile:
                self.log.error(
                    "can not find '%s', please check '%s' file" % (v, RC_COMMON_PACK))
    ...
    def _parseClientConfig(self, path):
        ...
        parser.set(
            "Server", "ErrorReportAddress", "http://reporting.volam2.zing.vn")
    ...
    def configClient(self):
        ...
        if not os.path.exists(RC_COMMON_PACK):
            ...
            self.log.info(
                "please run program with '-p', packaged programs and librarys!")
            return False
        if self._isLinux():
        ...
        if not os.path.isdir(resource):
            self.log.error(
                "can not find '%s', please check '%s' file" % (resource, self.config))
            return False
        catalog = self.configParser.get("client", "catalog").split(',')
        for clv in catalog:
            clvpath = os.path.join(resource, clv)
            self.copyFolder(clvpath, root)
        specialcatalog = self.configParser.get(
            "client", "special_catalog").strip()
        ...
        self._parseIniFile(
            root, "settings/server_list.ini", self._parseClientServerList)
    ...
    def _parseGoddessConfig(self, path):
        ...
        parser.set(
            'DatabaseServer', 'Server', self.configParser.get("mysql", "ip"))
        parser.set(
            'DatabaseServer', 'UserName', self.configParser.get("mysql", "username"))
        parser.set(
            'DatabaseServer', 'Database', self.configParser.get("mysql", "database"))
        parser.set(
            'DatabaseServer', 'Password', self.configParser.get("mysql", "password"))
    ...
    def _parseBishopConfig(self, path):
        ...
        parser.set(
            'Paysys', 'IPAddress', self.configParser.get("paysys", "ip"))
    ...
    def _parseRelayConfig(self, path):
        ...
        parser.set(
            'DataBase', 'DBName', self.configParser.get("mysql", "database"))
        parser.set('DataBase', 'LogDBName', "%s_log" %
                   self.configParser.get("mysql", "database"))
        parser.set(
            'DataBase', 'DBUser', self.configParser.get("mysql", "username"))
        parser.set(
            'DataBase', 'DBPwd', self.configParser.get("mysql", "password"))
        ...
        parser.set(
            'GlobalDatabase', 'Server', self.configParser.get("mysql", "ip"))
        parser.set('GlobalDatabase', 'Database', "jx2vn_global_db")
        parser.set(
            'GlobalDatabase', 'User', self.configParser.get("mysql", "username"))
        parser.set(
            'GlobalDatabase', 'Password', self.configParser.get("mysql", "password"))
        ...
        parser.set(
            'MyGlbDB', 'User', self.configParser.get("mysql", "username"))
        parser.set(
            'MyGlbDB', 'Password', self.configParser.get("mysql", "password"))
    ...
    def _parseRouterConfig(self, path, *param):
        if len(param) != 2:
            self.log.error(
                "can not get mainip and subip %s! abort." % str(param))
            exit()
        mainip = param[0]
        subip = param[-1]
        parser = ConfigParser.ConfigParser()
    ...
    def _configGRelay(self, dstPath, *param, **kw):
        self._extractFiles(dstPath, RC_RELAY_LIST)
        self._parseIniFile(
            dstPath, "relay.ini", self._parseGRelayConfig, *param, **kw)

    def _configRouter(self, dstPath, *param):
        self._extractFiles(dstPath, RC_ROUTER_LIST)
        self._parseIniFile(
            dstPath, "Router.ini", self._parseRouterConfig, *param)
    ...
    def _configGateway(self, name, root, resource, catalog, func, *param, **kw):
        ...
        if len(param) == 0:
            ...
            if not os.path.isdir(srcPath):
                ...
                self.copyFolder(srcPath, dstPath)
            special_catalog = self.configParser.get(
                "server", "special_catalog")
    ...
    def _parseGamesvr(self, path):
        ...
        parser.set(
            'Server', 'IS_KINGSOFT_INNER_VERSION_acc_jh34ji84r347e8T56', 0)
    ...
    def configServer(self):
        ...
        self._configGateway(
            "Goddess", root, resource, catalog, self._configGoddess)
        self._configGateway(
            "Bishop", root, resource, catalog, self._configBishop)
        self._configGateway(
            "Relay", root, resource, catalog, self._configRelay)
    ...
    def configRouter(self):
        ...
        self._configGateway(
            "Relay", root, resource, catalog, self._configGRelay, mainip, **mysql)
        self._configGateway(
            "Router", root, resource, catalog, self._configRouter, mainip, subip)
    ...
    def _configMysql(self, **kw):
        ...
        try:
            conn = MySQLdb.connect(
                host=kw["host"], user=kw["user"], passwd=kw["passwd"], port=kw["port"])
            cur = conn.cursor()
            ...
            if len(format) > 0:
                ...
                if (mainversion == 5 and subversion > 1) or mainversion < 5:
                    self.log.error(
                        "mysql version is ERROR!!!, Version 5.1.69 is best" % data)
                    cur.close()
                    conn.close()
                    return False
    ...
    def registerPaysys(self, name, passwd):
        ...
        self.log.info("==> register local machine to paysys(%s:%s/%s)" %
                      (ora_ip, ora_port, ora_server_name))
        ...
        try:
            ...
            if len(format) > 0:
                ...
                cursor.execute(sql % (
                    int(format[0]) + 1, name, md5pd, self.localIp, self.localMac, self.localIp))
            else:
                cursor.close()
                connection.close()
